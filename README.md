...
5. Импорт данных
   ...
   Для последовательного запуска импортов используется файл `imports.http`.
   
   Запросы в файле расположены в порядке, необходимом для корректного выполнения полного импорта.
   
   Если импорт зависит от результатов предыдущих импортов, его зависимости указаны в комментарии `Depends on` над соответствующим запросом. Это позволяет определить необходимые предварительные импорты при частичном запуске.